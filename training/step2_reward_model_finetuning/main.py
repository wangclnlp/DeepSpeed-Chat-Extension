...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=['length_ratio', 'cosine_similarity', 'random', 'fixed'],
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help='Value for \'fixed\' method for controlling scale of reward.')
    parser.add_argument(
        "--random_constraints_range",
        type=float,
        nargs=2,
        default=[1.0, 2.0],
        help='Value for \'random\' method for controlling scale of reward.')
    parser.add_argument(
        "--beta1_for_controlling_scale_of_reward",
        type=float,
        default=6,
        help=
        "Control delta_star=beta1*(similarity+beta2)."
    )
    parser.add_argument(
...
def main():
    ...
    if args.method_for_controlling_scale_of_reward == 'cosine_similarity':
        ref_model_name_or_path = 'models/google-bert/bert-base-cased/'
        ...
        ref_model = AutoModel.from_pretrained(ref_model_name_or_path)
        # End of DS config for ref model
        ref_model, *_ = deepspeed.initialize(model=ref_model, config=ref_ds_config)
        ref_model.eval()
        ref_train_phase = 2
        ref_train_dataset, ref_eval_dataset = create_prompt_dataset(
            args.local_rank, args.data_path, args.data_split,
            args.data_output_path, ref_train_phase, args.seed, ref_tokenizer,
            512, end_of_conversation_token="")

        # DataLoaders creation:
        ref_data_collator = DataCollatorReward()
        if args.local_rank == -1:
            ref_train_sampler = RandomSampler(ref_train_dataset)
        else:
            ref_train_sampler = DistributedSampler(ref_train_dataset)
        ref_train_dataloader = DataLoader(ref_train_dataset,
                                    collate_fn=ref_data_collator,
                                    sampler=ref_train_sampler,
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if args.method_for_controlling_scale_of_reward == 'cosine_similarity':
            assert len(ref_train_dataloader) == len(train_dataloader)
            ref_train_dataloader_iter = iter(ref_train_dataloader)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            ref_logits = None
            if args.method_for_controlling_scale_of_reward == 'cosine_similarity':
                ref_batch = to_device(next(ref_train_dataloader_iter), device)
                with torch.no_grad():
                    ref_output = ref_model(**ref_batch)
                batch['ref_output'] = ref_output
            batch['beta1_for_controlling_scale_of_reward'] = args.beta1_for_controlling_scale_of_reward
            batch['beta2_for_controlling_scale_of_reward'] = args.beta2_for_controlling_scale_of_reward
            batch['beta3_for_controlling_scale_of_reward'] = args.beta3_for_controlling_scale_of_reward
            batch['scale_control_method'] = args.method_for_controlling_scale_of_reward
            batch['random_constraints_range'] = args.random_constraints_range
            batch['fixed_constraints'] = args.fixed_constraints
            ...
            if torch.distributed.get_rank() == 0 and step%5==0:
                hours, rem = divmod((end-start)*(len(train_dataloader)-step-1), 3600)
                minutes, seconds = divmod(rem, 60)
                print("Time needed: {:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds), "Loss: {}".format(loss.item()), \
                      "Chosen: {}, Rejected:{}".format(outputs["chosen_mean_scores"][-1], outputs["rejected_mean_scores"][-1]), "Similarity: {}".format(outputs["similarity"]), "Delta_Star: {}".format(outputs["delta_star"]))
        ...
        if (epoch+1)%args.save_epochs==0 and args.output_dir is not None:
            save_model(rm_model, tokenizer, args, sub_folder="epoch-{}".format(epoch+1))

    if args.save_last:
        save_model(rm_model, tokenizer, args, sub_folder="last")
