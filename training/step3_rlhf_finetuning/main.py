...
def main():
    ...
    def save_model(sub_folder='', save_critic=False):
        print_rank_0('saving model ...')
        rlhf_engine.actor = convert_lora_to_linear_layer(rlhf_engine.actor)
        if save_critic: rlhf_engine.critic = convert_lora_to_linear_layer(rlhf_engine.critic)
        if args.enable_ema:
        ...
        if torch.distributed.get_rank() == 0:
            save_hf_format(rlhf_engine.actor,
                           tokenizer,
                           args,
                           sub_folder=os.path.join(sub_folder, 'actor'))
            if save_critic:
                save_hf_format(rlhf_engine.critic,
                               tokenizer,
                               args,
                               sub_folder=os.path.join(sub_folder, 'critic'))
            if args.enable_ema:
                save_hf_format(rlhf_engine.actor_ema,
                               tokenizer,
                               args,
                               sub_folder=os.path.join(sub_folder, 'actor_ema'))

        if args.actor_zero_stage == 3:
            save_zero_three_model(rlhf_engine.actor,
                                  global_rank=args.global_rank,
                                  save_dir=os.path.join(
                                      args.output_dir, sub_folder, 'actor'),
                                  zero_stage=args.actor_zero_stage)
            if args.enable_ema:
                save_zero_three_model(rlhf_engine.actor_ema,
                                      global_rank=args.global_rank,
                                      save_dir=os.path.join(
                                          args.output_dir, sub_folder, 'actor_ema'),
                                      zero_stage=args.actor_zero_stage)
        if args.critic_zero_stage == 3 and save_critic:
            save_zero_three_model(rlhf_engine.critic,
                                  global_rank=args.global_rank,
                                  save_dir=os.path.join(
                                      args.output_dir, sub_folder, 'critic'),
    ...
    for epoch in range(args.num_train_epochs):
        print_rank_0(
            ...
            args.global_rank)
        if global_step_count > args.num_train_steps: break
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if exp_dataset is not None:
                ...
                print_rank_0(
                    ...
                    args.global_rank)
                if torch.distributed.get_rank() == 0:
                    hours, rem = divmod((e2e_time)*(len(prompt_train_dataloader)-step-1), 3600)
