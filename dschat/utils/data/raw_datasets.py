...
class PromptRawDataset(object):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        if not any([d in dataset_name for d in ['alpaca_farm', 'tldr', 'summarize-from-feedback', 'Anthropic/hh-rlhf']]):
...
class AlpacaFarmPPODataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "tatsu-lab/alpaca_farm_instructions_ppo"
        self.dataset_name_clean = "tatsu_lab_alpaca_farm_instructions_ppo"
        self.raw_datasets = load_dataset('json',
                                         data_files={
                                             "train":
                                             dataset_name + '/unlabeled.json',
                                             "eval":
                                             dataset_name + '/val.json'
                                         })
...
class AnthropichhrlhfDataset(PromptRawDataset):
    ...
    def get_prompt(self, sample):
        c, r = sample['chosen'], sample['rejected']
        c_ = c.split('\n\nAssistant: ')
        r_ = r.split('\n\nAssistant: ')
        i = 0
        while c_[i] == r_[i]:
    ...
    def get_chosen(self, sample):
        chosen = sample['chosen'][len(self.get_prompt(sample)):]
        return chosen

    def get_rejected(self, sample):
        rejected = sample['rejected'][len(self.get_prompt(sample)):]
        return rejected
...
# TLDR dataset from https://github.com/openai/summarize-from-feedback
class TLDR3FilteredDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "openai/summarize-from-feedback-tldr_3_filtered"
        self.dataset_name_clean = "openai_summarize-from-feedback-tldr_3_filtered"
        self.raw_datasets = load_dataset('json',
                                         data_files={
                                             "train":
                                             dataset_name + '/train.jsonl',
...
# TLDR dataset from https://github.com/openai/summarize-from-feedback
class TLDR3FilteredHumanFeedbackDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "openai/summarize-from-feedback-human-feedback"
        self.dataset_name_clean = "openai_summarize-from-feedback-human_feedback"
        self.raw_datasets = load_dataset('json',
                                         data_files={
                                             "train":
                                             dataset_name + '/train.jsonl',
                                             "eval":
                                             dataset_name + '/valid1.jsonl'
                                         })
