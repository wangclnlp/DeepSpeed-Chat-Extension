...
from rlhf_llama.common import common_path
...
class PromptRawDataset(object):
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        self.raw_datasets = load_dataset("parquet", data_files={'train': f'{dataset_name}/train.parquet', 'test': f'{dataset_name}/test.parquet'})
...
class GPTAnnotationScoreRMDataset(PromptRawDataset):
    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = dataset_name
        self.dataset_name_clean = dataset_name
    ...
    def get_chosen_gpt_score(self, sample):
        return sample['chosen_gpt_score']
    
    def get_rejected_gpt_score(self, sample):
        return sample['rejected_gpt_score']

    def get_gpt_correctness(self, sample):
        return sample['gpt_correctness']
...
class MultiTurnAlpacaDataset:
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        self.dataset_name = dataset_name
        self.dataset_name_clean = dataset_name
        if "neu" in dataset_name:
            data_dict = {
                'train': dataset_name + '/train.txt',
                'test': dataset_name + '/test.txt',
            }
        elif "gaobao" in dataset_name:
            data_dict = {
                'train': dataset_name + '/train.txt',
                'test': dataset_name + '/test.txt',
            }
            ...
            data_dict = {
                'train': common_path.data_dir + 'training_data.txt.train',
                'test': common_path.data_dir + 'training_data.txt.test',
            }
        self.raw_datasets = DatasetDict.from_json(data_dict)
...
class MultiTurnSFTDataset:
    ...
    def get_history(self, sample):
        if 'history' in sample:
            return sample['history']
        else:
            return []

class MultiTurnRLHFDataset:
    def __init__(self, output_path, seed, local_rank, dataset_name):
        self.output_path = output_path
        self.seed = seed
        self.local_rank = local_rank
