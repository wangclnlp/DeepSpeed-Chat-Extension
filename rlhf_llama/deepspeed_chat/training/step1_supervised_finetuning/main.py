...
def main():
    ...
    if args.iterative_alignment:
        ...
        # load previous rlhf model
        print_rank_0("load previous rlhf model..................................")
        previous_rlhf_model = create_hf_model(AutoModelForCausalLM,
                                args.previous_rlhf_model,
                                tokenizer,
                                ds_config,
                                disable_dropout=args.disable_dropout)
        ...
        for n, p in previous_rlhf_model.named_parameters():
            previous_parameters_dict[n.replace("module.", "")] = p.detach().cpu().clone()

        del previous_rlhf_model
        gc.collect()
        ...
        if os.path.exists(f"{args.previous_round_before_sft_model}/previous_fisher.bin"):
            ...
            for n, p in model.named_parameters():
                with deepspeed.zero.GatheredParameters(p, modifier_rank=0):
                    ...
                    previous_fp = torch.Tensor(previous_parameters_dict[n.replace("module.", "")])
                    # compute the different between rlhf model and previous rlhf model.
                    ewc.fisher[n.replace("module.", "")] += ((fp - previous_fp) ** 2).mean().item()*args.ewc_mse_factor
                    #load reference model
                    ewc.reference_model_parameters[n.replace("module.", "")] = fp
